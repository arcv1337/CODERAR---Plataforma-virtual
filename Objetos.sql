...
FROM alumnos alu
INNER JOIN genero ge ON ge.id_genero = alu.id_genero;
...
DELIMITER $$
CREATE FUNCTION  verRegXAño ( periodo int)
RETURNS INT DETERMINISTIC 
BEGIN
    DECLARE cant INT;
    select count(id_alumno) into cant from inscripcion where Year(fecha_de_registro) = periodo;
    return cant;
END$$
DELIMITER ;
